...
class ShopifyClient(
    ...
    val moshi: Moshi =
            Moshi.Builder()
                    ...
                    .add(
                            ...
                            EnumJsonAdapter.create(FinancialStatus::class.java)
                                    .withUnknownFallback(FinancialStatus.UNKNOWN),
                    )
                    .build()
    ...
    inline fun <reified T> getElement(
        ...
        val map =
                adapter.fromJson(message) ?: throw IllegalStateException("Failed to parse response")

        return map[resourceKey]
                ?: throw IllegalStateException("Resource key '$resourceKey' not found in response")
    }

    inline fun <reified T, reified R> putElement(
            resourceKey: String,
            subUrl: String,
            body: R,
            params: Map<String, String?> = emptyMap(),
        ...
        val wrapper = mapOf(resourceKey to body)
        val requestType =
                Types.newParameterizedType(Map::class.java, String::class.java, R::class.java)
        val requestAdapter: JsonAdapter<Map<String, R>> = moshi.adapter(requestType)
        val requestJson = requestAdapter.toJson(wrapper)
        ...
        val responseType =
                Types.newParameterizedType(Map::class.java, String::class.java, T::class.java)
        val responseAdapter: JsonAdapter<Map<String, T>> = moshi.adapter(responseType)
        val map =
                responseAdapter.fromJson(message)
    ...
    inline fun <reified T> get(
        ...
        while (true) {
            ...
            // Check for next link
            val linkHeader =
                    response.headers["Link"]?.firstOrNull()
                            ?: response.headers["link"]?.firstOrNull()
            val nextLink =
            ...
            check(nextLink.startsWith(restClient.baseUrl)) {
                "Next link does not start with base URL: $nextLink"
            }
        ...
        val finalResults =
                if (requestedLimit != null) allResults.take(requestedLimit) else allResults
